...
import { Story, Business, Finale } from './PitchSections';
import {
  ArrowUpRight,
  ArrowRight,
  Check,
...
const pdfUrls: Record<string, string> = {
  'demo-1': new URL('../public/demo/demo-1.pdf', import.meta.url).href,
  'demo-2': new URL('../public/demo/demo-2.pdf', import.meta.url).href,
  'demo-3': new URL('../public/demo/demo-3.pdf', import.meta.url).href,
};
...
export function Comparison({
  ...
  const fields = guided
    ? FIELDS.filter((f) => ['premium', 'property', 'deductible'].includes(f.key))
    : FIELDS;
  ...
  function download() {
    ...
    const rowsHtml = fields
      ...
      .join('');
    const html = `<!doctype html><html lang="ru"><meta charset="utf-8"><title>Polis · Демонстрационное предложение</title><style>body{font:15px/1.6 Arial,sans-serif;color:#182a2b;margin:48px}table{width:100%;border-collapse:collapse}th,td{padding:16px;border:1px solid #cdd9d2;text-align:left;vertical-align:top}small{font-size:12px}h1{font-size:32px}p{white-space:pre-wrap}@media print{body{margin:12px}}</style><h1>Предложение для магазина «Точка»</h1><p><strong>ДЕМОНСТРАЦИЯ. ${guided ? 'Сокращённый пример: только три условия. ' : ''}Все компании, документы и условия вымышлены. Не является страховым предложением.</strong></p><p>${escape(demo.requirements)}</p><p>Выбранный вариант: ${escape(demo.offers.find((o) => o.id === recommendation)!.name)}. Условия сверены пользователем в демонстрации.</p><p>${escape(comment)}</p><table><thead><tr><th>Условие</th>${demo.offers.map((o) => `<th>${escape(o.name)}</th>`).join('')}</tr></thead><tbody>${rowsHtml}</tbody></table></html>`;
      ...
      {!compact && (
        <>
          ...
          <div className="demo-tools">
            ...
            <span>
              {reviewed.length} из {fields.length * demo.offers.length} условий проверено
            </span>
          </div>
        </>
      )}
          ...
          <div className="review-offers">
            {demo.offers.map((o, i) => (
              <label key={o.id}>
                ...
                />
                Я сверил {guided ? 'условия примера' : 'все условия'} «{o.name}»
              </label>
            ))}
          </div>
              ...
              <>
                ...
                {cell.evidence ? (
                  <>
                    ...
                    <a
                      className="pdf-link"
                      href={`${pdfUrls[cell.evidence.fileId]}#page=${cell.evidence.page}`}
                      target="_blank"
                      rel="noreferrer"
                    >
                      <FilePdf size={20} />
    ...
    <>
      ...
      {tourOpen && (
        <GuidedDemo
          onClose={() => {
            setTourOpen(false);
            document.getElementById('start-demo')?.focus({ preventScroll: true });
          }}
        />
      )}
      <main id="main">
        <section className="hero lp-container">
          <div className="hero-copy">
            ...
            <p>
              Polis — AI-ассистент страхового брокера. Сравнивайте предложения, проверяйте источники
              и готовьте решение для клиента.
            </p>
            <div className="hero-actions">
              <button id="start-demo" className="lp-button" onClick={() => setTourOpen(true)}>
                Начать демо <ArrowRight size={18} />
              </button>
        ...
        <section className="human-section lp-container">
          <div className="human-image">
            <img
              src={new URL('../public/images/broker-desk.webp', import.meta.url).href}
              width="1536"
              height="1024"
              loading="lazy"
        ...
        <section
          className="platform-demo lp-container"
          id="platform-demo"
          aria-labelledby="platform-title"
        >
          <div className="section-intro">
            <span className="pitch-eyebrow">04 / Платформа в работе</span>
            <h2 id="platform-title">
              Вот как выглядит
              <br />
              рабочий день с Polis.
            </h2>
            <p>
              Настоящий интерфейс кабинета на учебных данных: заявки, документы, сравнение,
              источники и экспорт. Без регистрации.
            </p>
          </div>
          <div className="workspace-preview">
            <div className="workspace-preview-bar">
              <span>Учебное пространство · изменения только в этой вкладке</span>
              <a href="/platform-demo.html" target="_blank" rel="noreferrer">
                Открыть на весь экран ↗
              </a>
            </div>
            <iframe
              id="workspace-demo-frame"
              src="/platform-demo.html"
              title="Рабочий кабинет Polis — интерактивный учебный пример"
              loading="lazy"
            />
          </div>
        </section>
