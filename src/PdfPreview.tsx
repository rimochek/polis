...
export default function PdfPreview({fileId,page:initialPage}:{fileId:string;page:number}){
 ...
 return <div className="pdf-source" data-source={`/api/documents/${fileId}`} data-ready={!loading&&!error}><div className="pdf-controls"><button className="icon-button" aria-label="Предыдущая страница PDF" disabled={page<=1||loading} onClick={()=>setPage(p=>p-1)}><ArrowLeft size={15}/></button><span>Страница {page}{total>0&&` из ${total}`}</span><button className="icon-button" aria-label="Следующая страница PDF" disabled={!total||page>=total||loading} onClick={()=>setPage(p=>p+1)}><ArrowRight size={15}/></button><button className="icon-button pdf-zoom" aria-label={zoom?'Уменьшить PDF':'Увеличить PDF'} aria-pressed={zoom} onClick={()=>setZoom(!zoom)}><MagnifyingGlassPlus size={17}/></button></div><div className="pdf-canvas-scroll">{loading&&<div className="pdf-loading" role="status"><span className="spinner small"/>Загрузка страницы…</div>}{error&&<p className="pdf-error" role="alert">{error}</p>}<canvas ref={ref} className={zoom?'zoomed':''} style={{visibility:loading||error?'hidden':'visible'}} aria-label={`Источник: страница ${page}`}/></div></div>;
}
