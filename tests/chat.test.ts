...
test('real chat sends only this case current PDFs and explicitly flags stale comparison',async t=>{
 ...
 const reply=await answerChat({c,cases:[c,{...fixture(),id:'other',client:'Other secret client'}],history:[],text:'Что изменилось?',dataDir:dir,config});assert.equal(reply.sources?.length,1);
});
test('workspace chat receives metadata only, filters invented links, never loads PDFs',async t=>{
 ...
 const reply=await answerChat({cases:[c],history:[],text:'Что делать?',dataDir:'nonexistent',config});assert.deepEqual(reply.caseIds,['one']);
});
test('provider failure does not fabricate a reply; demo is explicitly local',async t=>{
 ...
 await assert.rejects(()=>answerChat({c,cases:[c],history:[],text:'Вопрос',dataDir:'',config}),/срок токена истёк/);
 const demo=await answerChat({c:{...c,demo:true},cases:[c],history:[],text:'Вопрос',dataDir:'',config});assert.match(demo.text,/запрос к модели не выполнялся/);
});
