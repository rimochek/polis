// Exercise Vertex request shaping and failure handling with mocked responses.
...
const offer: Offer = {
  id: 'offer',
  name: 'Synthetic',
  documents: [{ id: 'pdf-1', name: 'sample.pdf', pages: 2, size: 100, version: 1, createdAt: '' }],
  cells: emptyCells(),
};
...
test('Vertex receives PDF and server-side token; citations remain unreviewed', async (t) => {
  t.mock.method(globalThis, 'fetch', async (url: string, init: RequestInit) => {
    ...
    assert.equal(sent.contents[0].parts[1].inlineData.mimeType, 'application/pdf');
    ...
    return new Response(
      JSON.stringify({
        candidates: [
          {
            finishReason: 'STOP',
            content: { parts: [{ text: JSON.stringify({ fields: fields() }) }] },
          },
        ],
  ...
  const r = await analyzeGemini(c, offer, Buffer.from('%PDF-test'), config);
  assert.equal(r.cells.sum.reviewed, false);
  assert.equal(r.cells.sum.evidence?.fileId, 'pdf-1');
...
test('Google errors do not echo sensitive upstream data', async (t) => {
  ...
  await assert.rejects(
    () => callGemini(config, {}),
    (e) =>
      e instanceof Error && e.message.includes('доступе') && !e.message.includes('test-secret'),
  );
});
test('project and region cannot redirect credentials to another host', () => {
  assert.throws(() => vertexEndpoint({ ...config, location: 'attacker.example' }));
  assert.throws(() => vertexEndpoint({ ...config, project: '../../bad' }));
