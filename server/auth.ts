// Password hashing, signed access cookies, and rotating refresh-token families.
...
export async function verifyPassword(password: string, encoded: string) {
  ...
  const digest = (await scrypt(password, Buffer.from(saltText, 'base64url'), 64)) as Buffer;
...
export function verifyAccessToken(token: string): AuthUser | null {
  ...
  try {
    const data = JSON.parse(unbase64(payload)) as { sub?: string; email?: string; exp?: number };
    if (!data.sub || !data.email || !data.exp || data.exp < Math.floor(Date.now() / 1000))
      return null;
...
export function clearAuthCookies(res: Response) {
  res.setHeader('Set-Cookie', [
    cookie(accessCookie, '', 0, true),
    cookie(refreshCookie, '', 0, true, '/api/auth'),
    cookie(csrfCookie, '', 0, false),
  ]);
}
...
export function accessUser(req: Request) {
  const token = cookies(req)[accessCookie];
  return token ? verifyAccessToken(token) : null;
}

export const requireAuth: RequestHandler = (req, res, next) => {
  const user = accessUser(req);
...
export const requireCsrf: RequestHandler = (req, res, next) => {
  ...
  const parsed = cookies(req)[csrfCookie];
  const header = req.headers['x-csrf-token'];
  if (!parsed || typeof header !== 'string' || parsed !== header)
    return res.status(403).json({ error: 'Проверка запроса не пройдена.' });
  req.csrfToken = parsed;
  next();
};

export async function startSession(user: AuthUser) {
  const refreshToken = newRefreshToken();
  const familyId = randomBytes(18).toString('hex');
...
// Rotate once inside a transaction; replaying a replaced token revokes its family.
export async function rotateSession(refreshToken: string) {
  const hash = hashRefreshToken(refreshToken);
  const rotated = await prisma.$transaction(async (tx) => {
    const current = await tx.authSession.findUnique({ where: { refreshTokenHash: hash } });
    ...
    const updated = await tx.authSession.updateMany({
      ...
      data: { revokedAt: now, replacedByTokenHash: nextHash },
    });
...
export function authErrorHandler(
  ...
  res
    .status(500)
    .json({ error: error instanceof Error ? error.message : 'Не удалось выполнить действие.' });
}
