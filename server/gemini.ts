...
export function vertexEndpoint(c: Config) {
  if (!/^gemini-[a-z0-9.-]+$/.test(c.model) || !/^([a-z0-9-]+)$/.test(c.location))
    throw new Error('Проверьте модель и регион Google Cloud.');
  const host =
    c.location === 'global'
      ? 'aiplatform.googleapis.com'
      : `${c.location}-aiplatform.googleapis.com`;
  if (c.express)
    return `https://aiplatform.googleapis.com/v1/publishers/google/models/${c.model}:generateContent`;
  if (!/^[a-z][a-z0-9-]{4,99}$/.test(c.project))
    throw new Error('Укажите ID проекта Google Cloud в настройках анализа.');
...
export async function callGemini(c: Config, body: unknown) {
  ...
  if (c.auth === 'token') headers.Authorization = `Bearer ${c.key}`;
  else headers['x-goog-api-key'] = c.key;
  try {
    for (let attempt = 0; ; attempt++) {
      response = await fetch(vertexEndpoint(c), {
        method: 'POST',
        headers,
    ...
    throw new Error(
      'Не удалось связаться с Google Cloud за отведённое время. Проверьте интернет и повторите анализ.',
    );
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(
      code === 401
        ? 'Google не принял ключ или срок токена истёк. Обновите подключение в настройках.'
        : code === 403
          ? 'Google Cloud отказал в доступе. Проверьте включение API, биллинг и разрешения ключа.'
          : code === 429
            ? 'Google Cloud: исчерпана квота или лимит. Повторите позже и проверьте квоты проекта.'
            : code === 404
              ? 'Модель недоступна в этом проекте. Проверьте название модели и регион.'
              : `Google Cloud вернул ошибку ${code}. Проверьте настройки модели и формат PDF.`,
    );
  }
...
export async function testGemini(c: Config) {
  const body = await callGemini(c, {
    contents: [{ role: 'user', parts: [{ text: 'Return exactly OK.' }] }],
    generationConfig: { maxOutputTokens: 1024 },
  });
  if (!body.candidates?.some((x) => x.content?.parts?.some((p) => p.text && !p.thought)))
...
export async function analyzeGemini(c: Case, offer: Offer, pdf: Buffer, config: Config) {
  ...
  if (candidate?.finishReason !== 'STOP')
    throw new Error('Gemini не завершил анализ документа. Попробуйте PDF меньшего объёма.');
  ...
  try {
    ...
    throw new Error('Gemini вернул некорректный ответ. Повторите анализ.');
  }
  return {
    cells: normalizeAnalysis(result, offer),
