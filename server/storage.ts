// S3-compatible PDF storage with deterministic keys derived from document IDs.
...
const endpointParts = (endpoint: string) => {
  ...
  if (url.pathname !== '/' || url.search || url.hash)
    throw new Error('STORAGE_ENDPOINT must contain only the protocol, host, and optional port.');
  return {
