...
import {z} from 'zod';
import {DOCUMENT_ROLES,caseQuestions,isCurrent,type Case,type ChatMessage} from '../shared/types.js';
import {answerChat,chatMessage} from './chat.js';

type Store={dataDir:string;list:()=>Case[];get:(id:string)=>Case;editable:(c:Case)=>void;touch:(c:Case,invalidate?:boolean,event?:string)=>void};
export function workspaceRoutes(store:Store){
 ...
 const workspaceFile=path.join(store.dataDir,'workspace-chat.json');
 let workspaceMessages:ChatMessage[]=fs.existsSync(workspaceFile)?JSON.parse(fs.readFileSync(workspaceFile,'utf8')):[];
 const chatBusy=new Set<string>();
 router.get('/chat',(_req,res)=>res.json(workspaceMessages));
 router.get('/cases/:id/chat',run((req,res)=>res.json(cFor(req).messages||[])));
 const chat=run(async(req,res)=>{
  const c=req.params.id?cFor(req):undefined;const key=c?.id||'workspace';
  ...
  const history=c?.messages||(!c?workspaceMessages:[]);const revision=c?.revision;
  chatBusy.add(key);
  try{
   const snapshot=c?structuredClone(c):undefined;
   const answer=await answerChat({c:snapshot,cases:store.list(),history:[...history],text:message,dataDir:store.dataDir});
   ...
   if(c){c.messages=next;store.touch(c);}else{workspaceMessages=next;fs.writeFileSync(workspaceFile+'.tmp',JSON.stringify(next));fs.renameSync(workspaceFile+'.tmp',workspaceFile);}
   res.json(next);
  }finally{chatBusy.delete(key);}
 });
 router.post('/chat',chat);router.post('/cases/:id/chat',chat);
 router.patch('/cases/:id/details',run((req,res)=>{
  ...
  Object.assign(c,input);store.touch(c,false,'Обновлены срок и ответственный');res.json(c);
 }));
 router.patch('/cases/:id/clarifications',run((req,res)=>{
  ...
  store.touch(c,false,`${input.resolved?'Отмечен ответ':'Возобновлён вопрос'}: ${q.offer}, ${q.title}`);res.json(c);
 }));
 router.put('/cases/:id/drafts',run((req,res)=>{
  ...
  c.drafts={...c.drafts,[input.offerId]:input.text};store.touch(c,false,'Сохранён черновик уточнений');res.json(c);
 }));
 ...
 router.post('/cases/:id/attachments',upload.single('file'),run(async(req,res)=>{
  ...
  store.editable(c);const id=randomUUID();fs.writeFileSync(path.join(store.dataDir,`${id}.pdf`),file.buffer);
  (c.attachments??=[]).push({id,name:Buffer.from(file.originalname,'latin1').toString('utf8'),role:documentRole,pages,size:file.size,version:1,createdAt:new Date().toISOString()});
  store.touch(c,true,`Добавлен документ: ${DOCUMENT_ROLES[documentRole]}`);res.json(c);
 }));
 router.patch('/cases/:id/attachments/:documentId',run((req,res)=>{
  ...
  doc.role=role.parse(req.body.role);store.touch(c,true,`Изменена роль документа «${doc.name}»`);res.json(c);
 }));
 return router;
}
