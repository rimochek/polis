"""Generate optional fictional presentation PDFs and a preview contact sheet."""
...
style = ParagraphStyle(
    "body", fontName="Arial", fontSize=11, leading=17, textColor=HexColor("#243a42")
)
offers = [
    (
        "01-Орбита.pdf",
        ...
        "#196a60",
        "420 000",
        "Помещение магазина, торговое оборудование и товарные запасы.",
        "100 000 тенге по каждому страховому случаю.",
        "Затопление вследствие аварии водопроводных и канализационных сетей включено в пределах общей страховой суммы. Отдельный подлимит не установлен.",
        "Естественный износ и постепенное воздействие влаги; умышленные действия страхователя.",
    ),
    (
        "02-Вектор.pdf",
        "Вектор Полис",
        "#315787",
        "365 000",
        "Помещение магазина и торговое оборудование. Товарные запасы не являются застрахованным имуществом.",
        "Безусловная франшиза: 1% от общей страховой суммы по каждому страховому случаю.",
        None,
        "Естественный износ; умышленные действия страхователя. Убытки в отношении товарных запасов не возмещаются.",
    ),
    (
        "03-Сфера.pdf",
        "Сфера Защита",
        ...
        "485 000",
        "Помещение магазина, торговое оборудование и товарные запасы.",
        "50 000 тенге по каждому страховому случаю.",
        "Затопление вследствие аварии водопроводных и канализационных сетей включено. Подлимит по данному риску: 5 000 000 тенге на весь срок страхования.",
        "Естественный износ и постепенное воздействие влаги; умышленные действия страхователя. По затоплению применяется подлимит раздела 2.",
    ),
]
for i, (
    filename,
    ...
    def page(number, title):
        ...
        c.rect(0, 810, 595, 32, fill=1, stroke=0)
        c.setFillColor(HexColor("#ffffff"))
        c.setFont("ArialBold", 9)
        c.drawString(42, 821, "УЧЕБНЫЙ ПРИМЕР / ВСЕ КОМПАНИИ И УСЛОВИЯ ВЫМЫШЛЕНЫ")
        c.setFillColor(HexColor("#19323b"))
        c.setFont("ArialBold", 23)
        c.drawString(42, 768, name)
        c.setFont("Arial", 10)
        c.drawString(42, 742, f"Предложение DEMO-2026-00{i} | 15 сентября 2026 года")
        c.setFont("ArialBold", 16)
        c.drawString(42, 702, title)
        c.setStrokeColor(HexColor("#d7e0e3"))
        c.line(42, 62, 553, 62)
        c.setFont("Arial", 8)
        c.setFillColor(HexColor("#607179"))
        c.drawString(
            42, 43, "Не является офертой, полисом или реальным страховым предложением."
        )
    ...
    block(
        "Страхователь",
        "Вымышленное ТОО «Точка Маркет». Объект: розничный магазин в Алматы.",
    )
    ...
    block(
        "Страховая премия",
        price + " тенге за весь срок страхования. Оплата единовременно.",
    )
    c.showPage()
    page(2, "2. Покрытие и особые условия")
    y = 666
    block(
        "Пожар",
        "Ущерб застрахованному имуществу вследствие пожара покрывается в пределах общей страховой суммы.",
    )
    if water:
    ...
    block(
        "Общие положения",
        "Условия действуют для указанного объекта и срока. Окончательный состав покрытия фиксируется в согласованном договоре и приложениях к нему.",
    )
    c.save()
...
for p in sorted(render.glob("[1-3]-[1-2].png")):
    ...
    tile = Image.new("RGB", (377, 535), "#e7ebed")
    tile.paste(im, (10, 20))
    ImageDraw.Draw(tile).text((10, 5), p.stem, fill="black")
    thumbs.append(tile)
sheet = Image.new("RGB", (377 * 3, 535 * 2), "white")
for i, im in enumerate(thumbs):
    sheet.paste(im, ((i // 2) * 377, (i % 2) * 535))
